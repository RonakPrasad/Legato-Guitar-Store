...
$fnameErr = $lnameErr = $passwordErr = $password1Err = $emailErr = "";
$fname = $lname = $email = $password = $password1 = "";
...
$specialChars = preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $password);
   ...
   if (empty($_POST["password1"])) {
    $password1Err = "You have to re-type the password";
  } else {
	  $password1=$_POST["password1"];
	  $lowercase = preg_match('@[a-z]@', $password1);
$number    = preg_match('@[0-9]@', $password1);
$specialChars = preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $password1);
	  if(!$lowercase || !$number || !$specialChars || strlen($password1) < 8) {
    $password1Err = 'Password should be at least 8 characters in length and should include at least one number, and one special character.';
  }if($password !=$password1)
	  $password1Err="Re-typed password does not match";
    else{
	$hashed_password1 = password_hash($password1, PASSWORD_DEFAULT);
  }
  }
  
  if(!$fnameErr && !$lnameErr && !$emailErr && !$passwordErr && !$password1Err && $password==$password1)
...
<title>Sign up - Legato Guitar Store</title>
...
<div class="container1" style="margin-top:40px;margin-left:65px;margin-right:65px;">
  ...
  <span style="font-size:50px;"><center>Create an Account</center></span>
   
       <label for="email">First Name</label>&nbsp; &nbsp; <span class="error">* <?php echo $fnameErr;?></span>
    <input type="text" name="fname" placeholder="Your first name..">
	
    <label for="email">Last Name</label>&nbsp; &nbsp; <span class="error">* <?php echo $lnameErr;?></span>
 ...
 <label for="password">Confirm Password</label> &nbsp; &nbsp;<span class="error">* <?php echo $password1Err;?></span>
    ...
    <br><br><br>
<big><b>Already have an Account?</b></big> <button style="padding:10px;"><a href="login.php" style="color:red;text-decoration:none;font-size:20px;"><b>Login</b></a></button>
</div>
<br><br><br>
